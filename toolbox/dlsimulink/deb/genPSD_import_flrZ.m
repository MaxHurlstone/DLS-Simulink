function PSD = genPSD_import_flrZ(par)

%% Generate Imported Disturbance PSD model
%%
data_import     = readcell('FloorY_PSD_A1.csv');
...
PSD_import_flrZ = interp1(data_frq, data_PSD, par.frq,'linear',0);         % Zero where outside of data frequency range

% Calculate signal power up to end of analysis frequency range
CPS_Mapped = cps(PSD_import_flrZ, par.frq);
idx        = find(data_frq > par.frq(end));
CPS_Import = cps(data_PSD(1:idx(1)), data_frq(1:idx(1)));

% Plot PSD for Floor vibrations
figure;
loglog(par.frq,PSD_import_flrZ(:))
hold on; grid
...
title('PSD Import Floor Vibrations Z')

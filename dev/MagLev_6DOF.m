%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%        Dynamic Error Budget Main        %%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% This is the starting script for performing a DEB analysis
% The disturbances models are propagated through the closed loop system, in a discrete representation
% Disturbance models are defined and imported as functions
% The closed loop system is contained in a state space form
% The closed loop transfer functions as well as PSD and CPS at the output are plotted

%% Initialise
%%
%clear all
close all
addpath('.\Tools','-end')                              % Locate tool library
addpath('.\Disturbances','-end')                       % Locate disturbance library
addpath('.\Mechatronic_System','-end')                 % Locate model directory

%% Set analysis parameters
%%
par.Res     = 5000;                                 % [-] Analysis resolution
par.Fmin	= 0.1;					 				% [Hz] bottom of analysis frequency range
par.Ts		= 1/2200;                               % [s] sample time of digital controller
par.Fs		= 1/par.Ts;								% [Hz] sample frequency
par.Fnyq	= 1/(2*par.Ts);	%100;						% [Hz] nyquist frequency
par.frq		= logspace(log10(par.Fmin),log10(par.Fnyq),par.Res)';	% [Hz] generate frequency grid

%% Create or import closed loop transfer function
%%
%TransF    = load('DeltaRobotMechatronicSystem_C1.mat');            % Load in .mat data from Simulink Linear Analysis Tool
TransF    = load('MAGLEV_6DOF_gravComp1600.mat');
G_cLoop   = TransF.LinearAnalysisToolProject.Results.Data.Value;   % Extract state space closed loop transfer functions 


%% ENTER DESIRED OUTPUT NUMBRE AND NUMBER OF ACTUATORS (NOISE INPUTS)
OUTPUT_NUM =1;
N = 6; % NUMBER OF ACTUATORS
dof_type = "lin"; % rot or lin
...
tf_flrx2xp = bode(G_cLoop(OUTPUT_NUM, 1 ),par.frq*2*pi);    % SS transfer function format: <Output number,Input number> 
tf_flry2xp = bode(G_cLoop(OUTPUT_NUM, 2 ),par.frq*2*pi);    % SS transfer function format: <Output number,Input number> 
tf_flrz2xp = bode(G_cLoop(OUTPUT_NUM, 3 ),par.frq*2*pi);    % SS transfer function format: <Output number,Input number> 
tf_amps2xp = squeeze(bode(G_cLoop(OUTPUT_NUM, 4:4+N-1 ),par.frq*2*pi));    % SS transfer function format: <Output number,Input number>
tf_sens2xp = squeeze(bode(G_cLoop(OUTPUT_NUM, 4+N:4+ 2*N -1 ),par.frq*2*pi));    % SS transfer function format: <Output number,Input number>
tf_DAC2xp = bode(G_cLoop(OUTPUT_NUM, 4+ 2*N ),par.frq*2*pi);    % SS transfer function format: <Output number,Input number>

figure;	
loglog(par.frq,tf_flrx2xp(:),'-', 'DisplayName',['flrx2xp [' err_unit '/m.s^-^2]']) % Transfer function using ANSYS SS block is in m/s2
hold on; grid
